...
static bool find_index(const Slice& haystack, const Slice& delimiter, int32_t part_number, Slice& res) {
    if (part_number > 0) {
        if (delimiter.size == 1) {
...
StatusOr<ColumnPtr> StringFunctions::substring_index(starrocks::FunctionContext* context, const starrocks::Columns& columns) {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        if (delimiter.size == 0) {
            ...
            if (find_index(haystack, delimiter, part_number, slice)) {
                res.append(slice);
            } else {
                res.append(haystack);
            }
